    ...
    <>
      <tr>
        <td className='flex gap-3 justify-start items-center font-latoRegular py-4 px-8'>
          <img src={child1} alt="imagen icono de persona" className='w-[1.5rem]' />
          {name}
        </td>
        ...
        <td className=' font-latoRegular py-4 px-6'>
          <button className='cursor-pointer'>
            <img src={studiesIcon} alt="icono de tabla que indica ver los estudios del paciente" className='w-[1.5rem] mx-auto' />
          </button>
        </td>
      </tr>
    </>
...
const Patients = () => {
  return (
    <main className='w-full gap-7 justify-center items-center'>
      <header className='w-fit font-baloo2 text-orange text-3xl font-black self-start ml-14 mt-6'>Perfil</header>
      <div className='flex flex-col gap-5 overflow-x-scroll scroll-smooth lg:overflow-hidden pb-7 mx-10'>
        <table class="table-auto m-5">
          <div className='table-header-group bg-lightBlue border-spacing-y-8 px-10'>
            <tr className=''>
              <th className=' font-baloo2 py-4 px-6 rounded-l-md text-center truncate'>Nombre y Apellido</th>
              <th className=' font-baloo2 py-4 px-6'>Obra Social</th>
              <th className=' font-baloo2 py-4 px-6'>Teléfono</th>
              <th className=' font-baloo2 py-4 px-6 text-center truncate'>Fecha de Nacimiento</th>
          ...
          <tbody className='m-5'>
            {
              PATIENTS.map((patient) => {
                return (
                  <TableRow name={patient.name} obraSocial={patient.obraSocial} tel={patient.tel} date={patient.date} />
                )
              })
            }
          </tbody>
